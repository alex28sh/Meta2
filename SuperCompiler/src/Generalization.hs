...
generalize (Con c ts) (Con c' ts') fv s1 s2 | c == c' && length ts == length ts' = 
    let (ts'', s1', s2') = foldr (\(t, t') (ts, s1, s2) -> 
            let (t'', s1'', s2'') = generalize t t' fv s1 s2 in 
                (t'' : ts, s1'', s2'')) ([], s1, s2) $ trace ("Generalizing Con: " ++ show (zip ts ts')) (zip ts ts') 
    in (trace ("Generalized Con: " ++ show (c, ts'')) $ Con c ts'', s1', s2')
generalize (App t u) (App t' u') fv s1 s2 | couple t t' =
    let (t'', s1', s2') = generalize t t' fv s1 s2
        (u'', s1'', s2'') = generalize u u' fv s1' s2'
    in (App t'' u'', s1'', s2'')
generalize u@(Lam x t) u'@(Lam x' t') fv s1 s2 | couple u u' =
...
generalize (Case t bs) (Case t' bs') fv s1 s2 | matchCase bs bs' =
    let (t'', s1', s2') = generalize t t' fv s1 s2  
        (bs'', s1'', s2'') = foldr (\((PCon name xs, b), (PCon name' xs', b')) (bs'', s1, s2) -> 
            let xs'' = renameVars (fv ++ map fst s1) $ trace ("Branch: " ++ show (xs, xs')) xs
                (b'', s1'', s2'') = generalize (foldr concrete b xs'') (foldr concrete b' xs'') (xs'' ++ fv) s1 s2
            in ((PCon name xs'', trace (show b'') $ foldl abstract b'' xs'') : bs'', s1'', s2'')) ([], s1', s2') $ trace ("Generalizing: " ++ show (zip bs bs')) (zip bs bs') 
    in (Case t'' $ trace ("Case: " ++ show (t'', bs'')) bs'', s1'', s2'')
...
renaming t u = trace ("Renaming: " ++ show (t, u)) $ renaming' t u []
